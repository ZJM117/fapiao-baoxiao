...
| 角色 | 看台账 | 入库建账 | 出报销单 | 改台账 | 重新识别 | 删台账行/清空 | 删已生成单据 | 审计日志 | 改设置 | 管账号 |
|---|---|---|---|---|---|---|---|---|---|---|
| 管理员 | 是 | 是 | 是 | 是 | 是 | 是 | 是 | 是 | 是 | 是 |
| 财务审核员 | 是 | 否 | 是 | 是 | 是 | 否 | 是 | 是 | 否 | 否 |
| 业务人员 | 是 | 是 | 是 | 是 | 否 | 否 | 否 | 否 | 否 | 否 |
| 只读用户 | 是 | 否 | 否 | 否 | 否 | 否 | 否 | 否 | 否 | 否 |

界面上藏起来的按钮只是「不显眼」，真正卡权限的地方在后端 —— **每个接口都会再查一次**，绕过界面直接发请求也越不了权。

> 这张表就是 `db.ROLE_ALLOW` 里的白名单本身，改了代码要同步改这里。
> 两处最容易记反：**财务审核员不能入库建账**；**业务人员不能重新识别、也不能删台账行**。

反过来也做了：**当前身份点不了的按钮会直接压灰**，并写明原因（悬停有提示，工具条下面还有一句
「当前登录：某某（角色）—— 灰掉的按钮这个身份不能用」）。以前是按钮亮着、点了才被后端拒绝，
用户只看到一句「删除失败」，既不知道是没权限、还是程序坏了。
...
## 金额口径与「按批次分开报」（2026-09-17）

**金额只有一个口径**：台账页顶部与统计卡的「合计」＝ 生成报销单时的「票据合计」。
打车行程单这类**发票的证明附件**，金额已经含在对应那张发票里，合计**不再加一遍**；
统计卡会多给一格「附件不重复计」，把排除掉的张数和金额摆出来，跟自己手算对得上。

**两批票分开报**：台账按「报销批次」分组，卡片上一批一枚胶囊
（张数 · 金额 · 日期区间 · 待报张数），点批次名只看这一批，点「全选这批」
把这一批勾上（附件自动跟主票，不用勾），然后照常生成单据 —— 两批各报各的。

**出行人姓名以票面为准**：高铁票 / 火车票票面上印着乘客姓名，解析器会读出来；
「设置出行人」批量填名字时，**票面写了姓名的行默认保护**、原样不动，
界面会列出「票面写的 vs 你要填的」，确认后才会覆盖（「出行人核对」面板里
逐行看过「来源」再改的，不拦）。

**Excel 只是快照**：台账的唯一可信源是 `发票台账.db`。导 Excel 时如果文件
被 Excel 占用，操作照样成功，界面只提示「Excel 快照没刷新」，关掉 Excel
后随便再改一次就会自动补上。

回归：`python 测试\diag_ledger_money.py`（用真实台账的只读副本验证口径与保护）。
